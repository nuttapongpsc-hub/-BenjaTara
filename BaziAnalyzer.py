...
class BaziAnalyzer:
    ...
    def find_useful_god(self, day_master, scores):
        ...
        if chart_type == "ดิถีแข็งแรง":
            useful_gods.append(self.generate[day_master])
            useful_gods.append(self.control[day_master])
        else:
            useful_gods.append(day_master)
            useful_gods.append(next(k for k, v in self.generate.items() if v == day_master))
